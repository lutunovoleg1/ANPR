...
def find_rectangle_corners(input_img, show_res=True):
    def filter_lines():
        """Categorizes detected vertical and horizontal lines into
        four groups based on their position relative to the image center.
           ...
           Returns:
               list: A list containing four sublists, where:
                   - line_positions[0] contains vertical line on the left side
                   - line_positions[1] contains horizontal line on the upper side
                   - line_positions[2] contains vertical line on the right side
                   - line_positions[3] contains horizontal line on the bottom side
           ...
           Note:
               ...
               (x1, y1, x2, y2) for vertical and horizontal lines.
           """
        central_point_h = (gray_h.shape[1] // 2, gray_h.shape[0] // 2)
        central_point_v = (gray_v.shape[1] // 2, gray_v.shape[0] // 2)

        line_positions = [[] for _ in range(4)]
        if v_lines is not None:
            for line in v_lines:
                # We refer to the right or left side by the x coordinate of the middle of the line
                # (if x>c_point[0], then right)
                x1, _, x2, _ = line[0]  # x2>x1
                if x1 + (x2 - x1) // 2 > central_point_v[0]:
                    line_positions[2].append(line)
                else:
                    line_positions[0].append(line)
        if h_lines is not None:
            for line in h_lines:
                # Relate to the upper or lower line by the y coordinate of the line's middle
                # (if y>c_point[1], then bottom)
                _, y1, _, y2 = line[0]  # y2>y1
                if y1 + (y2 - y1) // 2 > central_point_h[1]:
                    line_positions[3].append(line)
                else:
                    line_positions[1].append(line)

        # find the longest lines
        for i in range(len(line_positions)):
            line_positions[i] = sorted(line_positions[i], key=lambda val:
            ((val[0][2] - val[0][0]) ** 2 + (val[0][3] - val[0][1]) ** 2) ** 0.5, reverse=True)[:1]
        ...
        for i in range(len(line_positions)):
            if line_positions[i]:
                for line in line_positions[i]:
                    if i == 0 or i == 2:  # Select coefficient depending on the type of line
                        k = k_vertical
                    else:
                        k = k_horizontal
                    line[0][0] *= k[1]
    ...
    def find_intersection():
        for i in range(len(line_positions)):
            if line_positions[i] and line_positions[(i + 1) % 4]:  # Если две линии не пустые
                line_1, line_2 = line_positions[i][0], line_positions[(i + 1) % 4][0]
                ...
                if all([m1, m2, p1, p2]):  # проверка 0
                    ...
                    x = x1_1
                    y = (-a2 * x - c2) / b2
                elif p1 == 0 or p2 == 0:  # Если есть горизонтальная линия
    ...
    img_copy = cv2.resize(img_copy, (200, 80), cv2.INTER_LINEAR)  # Get one size fits all
    # Фильтрация изображения
    gray = cv2.cvtColor(img_copy, cv2.COLOR_BGR2GRAY)
    gray_f = cv2.bilateralFilter(gray, d=11, sigmaColor=200, sigmaSpace=100)

    # Изменяем разрешение для лучшего обнаружения вертикальной и горизонтальной линии номера
    gray_h = cv2.resize(gray_f, (150, 50))
    gray_v = cv2.resize(gray_f, (100, 150))
    ...
    thresh_v = cv2.morphologyEx(thresh_v, cv2.MORPH_ERODE, np.ones((15, 3)), iterations=1)

    # Использование Hough Transform для нахождения линий
    # Для собеля по у
    h_lines = cv2.HoughLinesP(thresh_h.astype(np.uint8), 1, np.pi / 180, threshold=30, minLineLength=30, maxLineGap=30)
    # Для собеля по х
    v_lines = cv2.HoughLinesP(thresh_v.astype(np.uint8), 1, np.pi / 180, threshold=30, minLineLength=20, maxLineGap=10)
    ...
    if show_res:
        ...
        stack_images_h = np.hstack((sobel_h, thresh_h))
        stack_images_v = np.hstack((sobel_v, thresh_v))
...
def prepare_result(ocr_res):
    num2char = {'0': 'О', '8': 'В'}
    char2num = {'О': '0', 'В': '8'}

    if ocr_res != []:
        # Сортируем результаты по координатам
        coords, texts, confs = list(zip(*ocr_res))
        text = ''.join(texts[:2])

        template_valid = False
        counter = 0
        if 8 <= len(text) <= 9:  # Проверка длинны номера
            for i in range(len(text)):
                if i in [0, 4, 5]:
                    if not text[i].isalpha():  # Обрабокта букв
                        text = text[:i] + num2char.get(text[i], text[i]) + text[i + 1:]
                    if text[i].isalpha():
                        counter += 1
                if i in [1, 2, 3, 6, 7, 8]:  # Обработка цифр
...
# img = cv2.imread('images_for_test/9.jpg')
# points = find_rectangle_corners(img)
